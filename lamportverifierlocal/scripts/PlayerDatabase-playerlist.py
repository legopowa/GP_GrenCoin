...
from brownie import network, web3, accounts, Wei, PlayerDatabase, Contract
...
class LamportTest:
    ...
    def __init__(self):
        ...
        self.contract = PlayerDatabase.at(contract_address)
    ...
    def can_test_key_functions(self, accs):
        ...
        address = "0x800065Da887de69bCCB75Bd0c1Fdc332C9540407"
        # steam_id = "76561197960269025"  # Replace with actual Steam Community ID
        # is_validator = True
        # is_registered = True
        # player_name = "PlayerOne"

        playerlist = _contract.getAllPlayerNames(
        ...
        print(playerlist)
        address = "0x026b0BCF6328F50b63aE33997381aaB008433fc4"
        # steam_id = "76561197960269025"  # Replace with actual Steam Community ID
        # is_validator = True
        # is_registered = True
        # player_name = "PlayerOne"

        # _contract.updateAnonIDContractAddress(
        ...
        Onboarded_filter = _contract.events.PlayerOnboarded.createFilter(fromBlock='latest')

        # Iterate through the events
        for event in Onboarded_filter.get_all_entries():
            # Access event data
            userAddress = event['args']['_address']
            steamID= event['args']['_steamID']
            isValidator = event['args']['_isValidator']
            isRegistered = event['args']['_isRegistered']
            playerName = event['args']['_playerName']
            # Print the data
            print(f"userAddress: {userAddress}, steamID: {steamID}, isValidator: {isValidator}, isRegistered: {isRegistered}, playerName: {playerName}")
        exit()