...
from brownie import network, web3, accounts, Wei, WhitelistTest, PlayerOnboardContract, AnonIDContract, Contract
...
class LamportTest:
    ...
    #     whitelist_test_contract_address = '0x09c7F5BB03497990AAE0a40dF2A6c87a15aaD430'  # Replace with actual address
        ...
        with open('contract_Onboard-coin.txt', 'r') as file:
            Onboard_contract_address = file.read().strip()
        print(f"Onboard Contract Address: {Onboard_contract_address}")


        # Access the deployed contracts
        anonid_contract = AnonIDContract.at(anonid_contract_address)
        contract = PlayerOnboardContract.at(Onboard_contract_address)
...
# Load mnemonics from files
        ...
        print(address2)
        print(address3)
        PlayerDatabase = '0x2c9A5B490f80622e8C0D00d06F2A94d0a832d334'
        #tx = whitelist_test_contract.addAddressToWhitelist(address, "anonID1", {'from': brownie_account})
        tx = contract.setPlayerDatabaseAddress(PlayerDatabase, {'from': brownie_account})
        #tx = contract.registerValidator(address2, {'from': brownie_account})
        #tx = contract.registerValidator(address3, {'from': brownie_account})
        # user added here                                  ^^^^^^^^^^^^^^^^^^^^
